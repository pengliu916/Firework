...
public:
	...
	{
		PrintInfo("MultiTexturePresenter:ModifyDeviceSettings...");
		...
		ID3D11ShaderResourceView* pInputTextureRV6 = NULL)
	{
		...
		V_RETURN(CompileShaderFromFile(L"MultiTexturePresenter.fx","VS","vs_4_0",&pVSBlob));
		V_RETURN(pd3dDevice->CreateVertexShader(pVSBlob->GetBufferPointer(),pVSBlob->GetBufferSize(),NULL,&m_pVertexShader));

		ID3DBlob* pGSBlob = NULL;
		V_RETURN(CompileShaderFromFile(L"MultiTexturePresenter.fx",GSname.c_str(),"gs_4_0",&pGSBlob));
		V_RETURN(pd3dDevice->CreateGeometryShader(pGSBlob->GetBufferPointer(),pGSBlob->GetBufferSize(),NULL,&m_pGeometryShader));
		pGSBlob->Release();

		ID3DBlob* pPSBlob = NULL;
		V_RETURN(CompileShaderFromFile(L"MultiTexturePresenter.fx","PS","ps_4_0",&pPSBlob));
